...
- `--no-git` - Disable git-aware tag suffixing and restore fallback (git context auto-detected from the repo of the path/cwd)
...
# Fail if any tag is missing (useful in CI)
...
- `--fail-on-miss` - Exit with error if any tag is not found
...
After setup, save/restore commands for that workspace will automatically encrypt/decrypt.
...
- `BORINGCACHE_NO_GIT` - Disable git-aware tag suffixing and restore fallback chain
...
# Mount with encryption (requires both recipient and identity)
