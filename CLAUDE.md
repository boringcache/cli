...
- `src/tag_utils.rs` - Platform-aware tag generation
...
1. **Save**: `path:tag` → platform detection → compression → archive → upload
2. **Restore**: `tag:path` → download → verify → extract → place files
3. **Platform tagging**: `user-tag` → `user-tag-platform-suffix` (e.g., `deps` → `deps-ubuntu-22.04-amd64`)
...
cargo test tag_utils::tests::test_tag_creation  
...
- Check `Platform::detect()` in `src/platform.rs:47`
- Validate tag suffix generation in `to_tag_suffix()` method
- Test platform suffix validation in `is_valid_tag_suffix()`
