...
stop_proxy
if ! verify_remote_tag_visible "$TMP_BINARY" "$WORKSPACE" "$TAG" "${LOG_DIR}/phase1b-publish" "$BUDGET_REMOTE_TAG_HITS_MIN" "${REMOTE_TAG_VERIFY_ATTEMPTS}" "${REMOTE_TAG_VERIFY_SLEEP_SECS}" "$PROXY_LOG"; then
  exit 1
fi
