...
pub mod cache {
    ...
    /// # Upload Contract
    ///
    /// If `exists == true`: Cache already exists, skip upload.
    ///
    /// If `exists == false`: Upload required. Backend returns:
    /// - `manifest_upload_url`: Presigned PUT URL for manifest (required)
    /// - `archive_part_urls`: Array of presigned PUT URLs for archive parts (required, 1..N)
    /// - `upload_id`: S3 multipart upload ID (required if parts > 1)
    ///
    /// For single-part uploads (1 URL), upload directly and use ETag from response.
    /// For multi-part uploads (N URLs), upload all parts then call complete endpoint.
    ...
    impl SaveResponse {
        /// Get archive upload URLs (handles both new and legacy field names)
        ...
        /// Get upload ID (handles both new and legacy field names)
    ...
    pub struct ConfirmRequest {
        ...
        #[serde(skip_serializing_if = "Option::is_none")]
        pub compressed_size: Option<u64>,
        /// Tag to bind when piggybacking on an existing upload
        #[serde(skip_serializing_if = "Option::is_none")]
        pub tag: Option<String>,
    }
    ...
    pub struct RestoreResult {
        ...
        #[serde(default)]
        pub error: Option<String>,
        /// Workspace's signing public key for server signature verification
        #[serde(default)]
        pub workspace_signing_public_key: Option<String>,
        /// Server-generated signature for this entry (base64 encoded)
        #[serde(default)]
        pub server_signature: Option<String>,
        /// ISO8601 timestamp when the entry was signed by the server
        #[serde(default)]
    ...
    pub struct CacheConfirmResponse {
        ...
        /// Tag binding status: "ready" (bound synchronously) or "pending" (deferred until upload completes)
        #[serde(skip_serializing_if = "Option::is_none")]
        pub tag_status: Option<String>,
        /// Server-generated signature for the cache entry (base64 encoded)
        #[serde(default)]
        pub signature: Option<String>,
        /// Workspace's signing public key (ed25519:base64 format)
        #[serde(default)]
        pub signing_public_key: Option<String>,
        /// ISO8601 timestamp when the entry was signed
        #[serde(default)]
