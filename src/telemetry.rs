...
/// Tigris storage metrics captured from response headers
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct StorageMetrics {
    /// Tigris region that served the request (from x-tigris-served-from header)
    pub region: Option<String>,
    /// Cache status: "hit", "miss", or "read" (from server-timing cache;desc=)
    pub cache_status: Option<String>,
    /// Block location: "local" or "remote" (from server-timing block;desc=)
    pub block_location: Option<String>,
    /// Full server-timing header for debugging
    pub timing_header: Option<String>,
}

impl StorageMetrics {
    /// Parse Tigris response headers into StorageMetrics
    ...
    /// Parse server-timing header to extract cache and block status
    /// Example: "total;dur=22,cache;desc=read;dur=2,block;desc=local;dur=19"
    fn parse_server_timing(header: &str) -> (Option<String>, Option<String>) {
        ...
        for part in header.split(',') {
            ...
            if part.starts_with("cache;") {
                // Extract desc=xxx from "cache;desc=read;dur=2"
                ...
                // Extract desc=xxx from "block;desc=local;dur=19"
                if let Some(desc_start) = part.find("desc=") {
                    let desc_value = &part[desc_start + 5..];
                    let end = desc_value.find(';').unwrap_or(desc_value.len());
...
impl TelemetryCollector {
    ...
    /// Set storage metrics from Tigris response headers
