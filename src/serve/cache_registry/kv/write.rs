...
pub(crate) fn kv_pending_publish_handoff_is_expired(handoff: &KvPendingPublishHandoff) -> bool {
...
pub(crate) fn kv_pending_publish_handoff_matches_state(
...
pub(crate) async fn read_kv_pending_publish_handoff(
    state: &AppState,
) -> Option<KvPendingPublishHandoff> {
...
pub(crate) async fn clear_kv_pending_publish_handoff(state: &AppState) {
    let path = kv_pending_publish_handoff_path(state);
...
pub(crate) async fn reconcile_kv_pending_publish_handoff(
    state: AppState,
    handoff: KvPendingPublishHandoff,
) {
