...
use crate::serve::cache_registry::{
    KvNamespace, RegistryError, await_startup_prefetch_readiness, do_download_blob_to_cache,
...
const TURBOREPO_MAX_CLIENT_CI_BYTES: usize = 50;
...
const TURBOREPO_HEADER_CLIENT_CI: &str = "x-artifact-client-ci";
const TURBOREPO_HEADER_CLIENT_INTERACTIVE: &str = "x-artifact-client-interactive";
...
pub(crate) async fn handle_artifact(
    ...
    ensure_proxy_bearer_header(headers).map_err(turborepo_error)?;
    validate_artifact_hash(hash).map_err(turborepo_error)?;
    match method {
        Method::PUT => {
            validate_required_content_length(headers).map_err(turborepo_error)?;
        ...
        read_method @ (Method::GET | Method::HEAD) => {
            if read_method == Method::GET {
                validate_artifact_client_headers(headers).map_err(turborepo_error)?;
            }
...
pub(crate) async fn handle_events(
    ...
    if bytes.iter().all(|byte| byte.is_ascii_whitespace()) {
        return Err(turborepo_error(RegistryError::bad_request(
            "Events request body must be a JSON array",
        )));
    }
    ...
    for event in &events {
        ...
        uuid::Uuid::parse_str(&event.session_id).map_err(|_| {
            turborepo_error(RegistryError::bad_request(
                "Event sessionId must be a valid UUID",
            ))
        })?;
...
fn validate_artifact_hash(hash: &str) -> Result<(), RegistryError> {
    if hash.is_empty() || !hash.bytes().all(|byte| byte.is_ascii_hexdigit()) {
        return Err(RegistryError::bad_request(
            "Turborepo artifact hash must match ^[a-fA-F0-9]+$",
        ));
    }
    Ok(())
}
...
fn validate_required_content_length(headers: &HeaderMap) -> Result<(), RegistryError> {
    let Some(value) = headers.get(header::CONTENT_LENGTH) else {
        return Err(RegistryError::bad_request(
            "Turborepo artifact uploads require Content-Length",
        ));
    };
    value
        .to_str()
...
fn validate_artifact_client_headers(headers: &HeaderMap) -> Result<(), RegistryError> {
    if parse_optional_string_header(headers, TURBOREPO_HEADER_CLIENT_CI)?
        .as_ref()
        .is_some_and(|value| value.len() > TURBOREPO_MAX_CLIENT_CI_BYTES)
    {
        return Err(RegistryError::bad_request(format!(
            "{TURBOREPO_HEADER_CLIENT_CI} exceeds {TURBOREPO_MAX_CLIENT_CI_BYTES} bytes"
        )));
    }

    if let Some(value) = parse_optional_string_header(headers, TURBOREPO_HEADER_CLIENT_INTERACTIVE)?
        && value != "0"
        && value != "1"
    {
        return Err(RegistryError::bad_request(format!(
            "{TURBOREPO_HEADER_CLIENT_INTERACTIVE} must be 0 or 1"
        )));
    }
...
fn parse_artifact_metadata(
    ...
    validate_artifact_client_headers(headers)?;
    let tag = parse_optional_string_header(headers, TURBOREPO_HEADER_TAG)?;
    if let Some(value) = tag.as_deref() {
        if value.len() > TURBOREPO_MAX_TAG_BYTES {
            return Err(RegistryError::bad_request(format!(
                "{TURBOREPO_HEADER_TAG} exceeds {TURBOREPO_MAX_TAG_BYTES} bytes"
            )));
        }
        STANDARD.decode(value).map_err(|_| {
            RegistryError::bad_request(format!("{TURBOREPO_HEADER_TAG} must be base64-encoded"))
        })?;
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn handle_events_accepts_empty_array() {
        let mut headers = HeaderMap::new();
        headers.insert("authorization", HeaderValue::from_static("Bearer token"));
        let response = handle_events(Method::POST, &headers, Body::from("[]"))
            .await
            .expect("events handler should accept empty event array");
        assert_eq!(response.status(), StatusCode::OK);
    }

    #[tokio::test]
    async fn handle_events_rejects_empty_body() {
        let mut headers = HeaderMap::new();
        headers.insert("authorization", HeaderValue::from_static("Bearer token"));
        let response = handle_events(Method::POST, &headers, Body::empty()).await;
