...
impl ChunkDownloader {
    ...
    pub async fn reassemble_and_verify(
        ...
        let computed_norm = computed_digest
            .strip_prefix("blake3:")
            .unwrap_or(&computed_digest);

        let manifest_norm = manifest
            .root
            .digest
            .strip_prefix("blake3:")
            .unwrap_or(&manifest.root.digest);

        if !computed_norm.eq_ignore_ascii_case(manifest_norm) {
            anyhow::bail!(
        ...
        if let Some(expected_hash) = expected_content_hash {
            let expected_norm = expected_hash
                .strip_prefix("blake3:")
                .unwrap_or(expected_hash);
            if !computed_norm.eq_ignore_ascii_case(expected_norm) {
                anyhow::bail!(
    ...
    async fn download_chunks_parallel(
        ...
        for (index, (digest, _size)) in unique_chunks.iter().enumerate() {
            ...
            futs.push(tokio::spawn(async move {
                ...
                let short_digest = &digest.split(':').next_back().unwrap_or(&digest)
                    [..12.min(digest.len())];
                ...
                let actual_hash = format!("blake3:{}", blake3::hash(&uncompressed_data).to_hex());
