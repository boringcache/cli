...
mod tests {
    ...
    async fn test_retry_max_retries_reached() {
        ...
        assert_eq!(call_count.load(std::sync::atomic::Ordering::SeqCst), 3); // max_retries
        assert!(start_time.elapsed() >= Duration::from_secs(1)); // At least 1+2 = 3 seconds delay
        assert!(result
            .unwrap_err()
            .to_string()
    ...
    fn test_resume_info_edge_cases() {
        ...
        resume_info.mark_chunk_complete(5, 100); // Invalid index
        assert_eq!(resume_info.downloaded_size, original_size); // Should not change
    }
}
