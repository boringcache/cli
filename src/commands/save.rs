...
async fn save_single_entry(
    ...
    // Storage metrics from upload response (captured for future metrics reporting)
    let mut _upload_storage_metrics = StorageMetrics::default();
    ...
    if !needs_upload {
        ...
        if is_multipart {
            ...
            (archive_etag, _upload_storage_metrics) = upload_archive_multipart(
            ...
            (archive_etag, _upload_storage_metrics) = upload_archive_file(
                final_archive_path.as_ref(),
                &archive_urls[0],
                &progress,
                api_client.transfer_client(),
    ...
    let confirm_request = ConfirmRequest {
        ...
        // Bind the tag during confirmation as well to avoid untagged/unknown entries
        tag: Some(tag.clone()),
    };
...
async fn upload_archive_multipart(
    ...
    let (uploaded_parts, storage_metrics) =
