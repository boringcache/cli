//! Check command - verifies tag existence on the server without downloading
...
/// Result of checking a single tag
...
pub async fn execute(
    ...
    // Build list of all candidate tags to check
    ...
    // Call the restore API to check tag existence
    let resolution_result = api_client.restore(&workspace, &all_candidates).await?;

    // Build a map of tag -> result for quick lookup
    ...
    // Process results for each requested tag
    ...
    for (requested_tag, candidates) in &tag_to_candidates {
        ...
        if let Some((matched_tag, entry)) = found {
            ...
            misses += 1;
            // Use the first candidate (most specific) as the tag name for misses
    ...
    // Collect missing tags for potential error message (before moving check_results)
