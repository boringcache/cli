...
async fn process_single_restore(
    ...
    ensure_empty_target(&target_path).await?;
...
async fn ensure_empty_target(path: &str) -> Result<()> {
    ...
    match fs::metadata(target).await {
        Ok(metadata) => {
            ...
            if dir_entries.next_entry().await?.is_some() {
                anyhow::bail!(
                    "Restore target '{}' is not empty. Please restore into an empty directory or remove the existing contents.",
                    target.display()
                );
            }
        }
