...
pub async fn execute_batch_restore(
    ...
    let resolution_result = match api_client.restore(&workspace, &all_candidates).await {
        Ok(result) => result,
        Err(err) => {
            reporter.step_complete(session_id.clone(), 1, step_start.elapsed())?;

            if crate::error::is_connection_error(&err) {
                reporter.session_error(session_id, format!("Cache unavailable: {}", err))?;
                drop(reporter);
                progress_system.shutdown()?;
                ui::warn(&format!("Cache unavailable, skipping restore: {}", err));
                return Ok(());
            }
            return Err(err);
        }
    ...
    for task in tasks {
        match task.await {
            Ok(Ok(RestoreOutcome::Restored {
                ...
                // Send storage and timing metrics to API
                let metrics = crate::api::MetricsParams {
                    ...
                    download_duration: Some(download_duration_ms),
                    extract_duration: Some(extract_duration_ms),
                    confirm_duration: None,
                    uncompressed_size: None,
                    compressed_size: Some(bytes_downloaded),
                    ...
                    part_count: None,
                    retry_count: None,
                    transfer_size: Some(bytes_downloaded),
                    cache_efficiency: None,
                    storage_region: storage_metrics.region,
                    storage_cache_status: storage_metrics.cache_status,
                    storage_block_location: storage_metrics.block_location,
                    storage_timing: storage_metrics.timing_header,
                };
    ...
    let finalize_result = finalize_restore_outcome(restore_errors, skipped_entries);
...
async fn process_restore(
    ...
    // Run manifest fetch and archive HEAD in parallel
    // The HEAD request warms the CLI's Tigris edge for the upcoming download
    ...
    // HEAD warms CLI's Tigris edge - capture metrics but don't fail on error
    ...
    // Use size from parallel HEAD request, fallback to expected
    let archive_size = expected_compressed;

    // Log storage metrics from HEAD warm request
    ...
    // Keep TempPath alive until after extraction to prevent automatic file deletion
    ...
    // Clean up decrypted temp file now that extraction is complete
    ...
    match extraction_result {
        Ok(()) => {
            ...
            // Display storage metrics in verbose mode
            if verbose {
            ...
            let download_speed_mbps = if download_elapsed.as_secs_f64() > 0.0 {
                Some(
                    (bytes_downloaded as f64 * 8.0)
                        / (download_elapsed.as_secs_f64() * 1_000_000.0),
                )
            } else {
                None
            };
    ...
    for (idx, task) in tasks.into_iter().enumerate() {
        match task.await {
            Ok(Ok((bytes, metrics))) => {
                ...
                // Capture metrics from first successful part
...
async fn download_sequential(
    ...
    // Capture storage metrics from GET response headers (the real source of truth)
...
async fn download_range(
    ...
    // Capture storage metrics from range request headers
    let storage_metrics = StorageMetrics::from_headers(response.headers());
