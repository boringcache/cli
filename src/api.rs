...
impl ApiClient {
                    ...
                    if !should_retry {
                        if err.is_connect() {
                            return Err(BoringCacheError::ConnectionError(
                                format!("ERROR: Cannot connect to BoringCache server after {attempt} attempts.\n\
                                        Please check:\n\
    ...
    pub async fn batch_check_existence(
        ...
        let url = self.build_workspace_url(workspace, "/caches/check")?;
        let request_body = serde_json::json!({
            "entries": entries.join(",")
        });

        let response = self.post(&url, &request_body).await?;
