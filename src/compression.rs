...
impl CompressionBackend {
    ...
    pub fn select_intelligent(
        ...
        if std::env::var("BORINGCACHE_DEBUG").is_ok() {
            eprintln!(
                "System specs: {} cores, {:.1}GB RAM, {:.1}% load, high_perf: {}",
                system.cpu_cores,
        ...
        match Self::analyze_context(size_mb, file_count, system, is_ci, is_container) {
            CompressionDecision::FavorZstd(reason) => {
                if std::env::var("BORINGCACHE_DEBUG").is_ok() {
                    eprintln!("Selecting ZSTD: {reason}");
                }
                CompressionBackend::Zstd
            }
            CompressionDecision::FavorLz4(reason) => {
                if std::env::var("BORINGCACHE_DEBUG").is_ok() {
                    eprintln!("Selecting LZ4: {reason}");
                }
                CompressionBackend::Lz4
            }
        }
