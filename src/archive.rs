...
pub async fn extract_tar_archive(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        extract_archive_optimized(reader, &target_path_owned)
    })
    .await
...
fn extract_archive_optimized<R: Read>(reader: R, target_path: &Path) -> Result<()> {
    ...
    for entry in archive.entries()? {
        ...
        sanitize_entry_path(&entry_path)
            .with_context(|| format!("Archive entry has unsafe path: {}", entry_path.display()))?;
        ...
        if matches!(
            ...
            if let Some(link_name) = entry.link_name()? {
                let link_path = link_name.into_owned();
                sanitize_entry_path(&link_path).with_context(|| {
                    format!(
                        "Archive entry links to unsafe path: {} -> {}",
                        entry_path.display(),
                        link_path.display()
                    )
                })?;
...
fn sanitize_entry_path(path: &Path) -> Result<()> {
    ...
    for component in path.components() {
        match component {
            ...
            Component::ParentDir => {
                anyhow::bail!("path traversal detected ('..' component)");
            }
    ...
    if path.as_os_str().is_empty() {
        anyhow::bail!("empty path detected in archive");
    }
