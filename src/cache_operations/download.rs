...
fn validate_download_hash(expected_hash: &str, actual_hash: &str, size_bytes: usize) -> Result<()> {
    ...
    anyhow::bail!(
        "Cache integrity verification failed.\n\
         ...
         Possible causes:\n\
         • Saved artifact was modified or replaced\n\
         • Non-deterministic or incompatible build output\n\
         • Network or storage corruption\n\n\
         Aborting restore to avoid cache poisoning. Re-save the cache or inspect the upstream artifact before retrying.",
...
fn resumable_temp_path(target_path_hint: Option<&str>) -> std::path::PathBuf {
    if let Some(path) = target_path_hint {
        let expanded = crate::commands::utils::expand_tilde_path(path);
        std::path::PathBuf::from(expanded)
    } else {
        let mut temp_path = std::env::temp_dir();
        temp_path.push(format!("boringcache-download-{}", Uuid::new_v4()));
        temp_path
    }
}
...
impl DownloadOperation {
    ...
    pub async fn download(
        ...
        let (downloaded_data, actual_hash) = if use_multipart {
            let temp_path = resumable_temp_path(target_path_hint);
        ...
        if let Some(expected_hash) = expected_content_hash {
            validate_download_hash(expected_hash, &actual_hash, downloaded_data.len())?;
        }
...
#[cfg(test)]
mod tests {
    use super::validate_download_hash;
    use tempfile::TempDir;

    #[test]
    fn resumable_temp_path_uses_hint_directly() {
        let hint = "./cache_target";
        let path = super::resumable_temp_path(Some(hint));
        assert_eq!(path, std::path::PathBuf::from(hint));
    }

    #[test]
    fn resumable_temp_path_without_hint_uses_system_temp_dir() {
        let path = super::resumable_temp_path(None);
    ...
    #[test]
    fn hash_validation_allows_matching_hashes() {
        assert!(validate_download_hash("abc", "abc", 42).is_ok());
    }

    #[test]
    fn hash_validation_rejects_mismatched_hashes() {
        let error = validate_download_hash("abc", "def", 42).unwrap_err();
        let message = format!("{}", error);
        assert!(message.contains("Cache integrity verification failed"));
