...
///
/// Provides system resource detection for optimal cache operation tuning
/// including memory, CPU, and disk performance characteristics.
...
impl SystemResources {
    ...
    pub fn recommended_download_concurrency(&self, is_ci: bool) -> usize {
        ...
        // Memory pressure: avoid too many simultaneous buffers.
        ...
        // Disk throughput gating: conservative defaults on slower disks.
        concurrency = match self.disk_type {
            DiskType::NvmeSsd => concurrency.min(12),
            DiskType::SataSsd => concurrency.min(4),
        };

        // If host is already hot, back off by one.
        if self.cpu_load_percent > 75.0 && concurrency > 1 {
            concurrency -= 1;
        }

        if is_ci {
            // CI runners are typically low core-count; keep contention low.
            concurrency = concurrency.min(4);
            if self.cpu_cores <= 2 {
                concurrency = concurrency.min(2);
            }
        } else {
            // Locally we can afford a little more aggressive scheduling.
