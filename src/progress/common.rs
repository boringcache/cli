/// Common progress handling utilities and patterns
///
/// Provides reusable progress tracking components for upload, download,
/// and other long-running operations to reduce duplication.
...
impl TransferProgress {
    ...
    pub fn record_bytes(&self, bytes: u64) -> Result<()> {
        ...
        let speed_mb_s = if elapsed.as_secs_f64() > 0.0 {
            total_transferred as f64 / (1024.0 * 1024.0) / elapsed.as_secs_f64()
        } else {
            0.0
        };
    ...
    pub fn complete(&self) -> Result<()> {
        ...
        self.record_bytes(0)?; // Update to 100%
        Ok(())
    }
...
/// Progress session manager for multi-step operations
...
/// Handle for a specific step in a progress session
...
mod tests {
    ...
    fn test_progress_format_percent() {
        ...
        assert_eq!(ProgressFormat::format_percent(150.0), "100%"); // Clamped
    }
}
