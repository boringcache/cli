...
use crate::api::ConfirmUploadParams;
...
pub fn create_confirm_params(
    ...
    ConfirmUploadParams {
        ...
        file_count: Some(archive_info.file_count),
        storage_key,
...
pub async fn download_file(
    ...
    _progress_bar: Option<&()>, // Kept for API compatibility but unused
) -> Result<()> {
    let client = Client::new();
    let response = http_request_with_validation(&client, client.get(url), "Download file").await?;
...
pub fn calculate_chunks(total_size: u64) -> Vec<(u64, u64)> {
    let optimal_chunk_size = 32 * 1024 * 1024;
