...
| Proxy runtime and readiness | same proxy command plus runtime status endpoint behavior | `support-primary`, `internal-only` | `src/serve/runtime/mod.rs`, `src/serve/runtime/listener.rs`, `src/serve/runtime/maintenance.rs`, `src/serve/runtime/shutdown.rs`, `src/serve/http/routes.rs`, `src/serve/http/handlers/mod.rs` | `AppState`, `BlobReadCache`, `PrefetchMetrics`, maintenance loop, shutdown handoff | `tests/serve_tests.rs`, `docs/proxy-mode.md`, `docs/contracts/readiness.md`, `docs/performance-learning-log.md` | Backs shared readiness state, the hidden ready-file orchestration handoff, `/_boringcache/status`, on-demand bypass, and shutdown publish behavior. OCI publish fallback warnings include the underlying phase error instead of only the generic status. Status snapshots expose OCI body-plane local hits, remote fetches, bytes, duration, and startup hydration counts for diagnostics. |
| Registry protocol dispatch | proxy-side OCI/Bazel/Gradle/Maven/Nx/Turborepo/sccache/Go protocol handling | `support-primary`, `internal-only` | `src/serve/cache_registry/mod.rs`, `src/serve/cache_registry/route.rs`, `src/serve/cache_registry/bazel.rs`, `gradle.rs`, `maven.rs`, `nx.rs`, `turborepo.rs`, `sccache.rs`, `go_cache.rs`; OCI invariants in `src/serve/engines/oci/**` | `cache_ops`, `kv_publish`, `upload_sessions`, `blob_read_cache`, `AppState`, `serve::engines::oci` | broad protocol coverage in `tests/serve_tests.rs`, context in `docs/adapter-cache-profiles.md`, source-rooted plan in `docs/adr/0002-proxy-engine-plan-b.md` | This is the heart of the proxy feature, but not a separate CLI surface. OCI selected-ref startup has three body policies: metadata-only, bodies-before-ready, and bodies-background. Non-OCI adapters keep the existing KV proxy behavior. OCI upload session semantics now live in `serve::engines::oci::uploads`: start, PATCH, closing PUT, mount `201`/`202`, empty finalize reuse, stale offset `416`, and streaming digest verification for one-shot upload bodies; the HTTP upload handler maps typed outcomes to responses and adds digest-valued `ETag` headers for immutable OCI responses. OCI blob upload planning, per-blob upload, and batch upload completion emit JSONL observability events. OCI blob body reads retry transient storage/server responses before returning a registry 500 to BuildKit. OCI registry mounts are publish-affecting: returning 201 for `mount=` must stage a finalized target upload session from existing local/prefetched bytes, otherwise manifest validation cannot prove mounted descriptors to the backend. OCI manifest publish now proves every descriptor through `upload-session`, `mounted-session`, `manifest-reference-session`, `local-body-cache`, or `remote-storage`; local body-cache hits are materialized into upload sessions so backend publish can still upload bytes when needed. Upload job selection consumes the exact `PresentBlob` proof session id instead of reselecting sessions globally by digest; a `remote-storage` proof cannot be used for an upload request without local bytes. |
