...
4. use [`boringcache/one@1d01e5dbf19ce259f921aa353d5e3e4ac5f942e4`](https://github.com/boringcache/one) in GitHub Actions
...
```yaml
- uses: boringcache/one@1d01e5dbf19ce259f921aa353d5e3e4ac5f942e4 # v1.19.0
  with:
    trust-policy: auto
    setup: none
    mode: archive
...
For proxy-backed modes, `boringcache/one@1d01e5dbf19ce259f921aa353d5e3e4ac5f942e4` also accepts first-class `metadata-hints` so sessions and misses stay grouped by stable labels instead of per-run noise:
...
If the repo already defines `[proxy]` or adapter `metadata-hints` in `.boringcache.toml`, `boringcache/one@1d01e5dbf19ce259f921aa353d5e3e4ac5f942e4` inherits them through the CLI dry-run plan. Prefer repo config for durable defaults and use the action input only when the workflow needs an explicit override.
...
Use `boringcache/one@1d01e5dbf19ce259f921aa353d5e3e4ac5f942e4` when you want the action to keep owning tool setup such as Bazel rc files, Maven or Gradle cache config, buildx setup, or container networking.
