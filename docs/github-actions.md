...
4. use [`boringcache/one@e24257b122813ad11d53b9ed024b474ca4946ad2`](https://github.com/boringcache/one) in GitHub Actions
...
For proxy-backed modes, `boringcache/one@e24257b122813ad11d53b9ed024b474ca4946ad2` also accepts first-class `metadata-hints` so sessions and misses stay grouped by stable labels instead of per-run noise:
...
If the repo already defines `[proxy]` or adapter `metadata-hints` in `.boringcache.toml`, `boringcache/one@e24257b122813ad11d53b9ed024b474ca4946ad2` inherits them through the CLI dry-run plan. Prefer repo config for durable defaults and use the action input only when the workflow needs an explicit override.
...
Use `boringcache/one@e24257b122813ad11d53b9ed024b474ca4946ad2` when you want the action to keep owning tool setup such as Bazel rc files, Maven or Gradle cache config, buildx setup, or container networking.
