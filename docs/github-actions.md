...
4. use [`boringcache/one@bf810e34331db84f9f11930e83b8813b5ad31ba1`](https://github.com/boringcache/one) in GitHub Actions
...
For proxy-backed modes, `boringcache/one@bf810e34331db84f9f11930e83b8813b5ad31ba1` also accepts first-class `metadata-hints` so sessions and misses stay grouped by stable labels instead of per-run noise:
...
If the repo already defines `[proxy]` or adapter `metadata-hints` in `.boringcache.toml`, `boringcache/one@bf810e34331db84f9f11930e83b8813b5ad31ba1` inherits them through the CLI dry-run plan. Prefer repo config for durable defaults and use the action input only when the workflow needs an explicit override.
...
Use `boringcache/one@bf810e34331db84f9f11930e83b8813b5ad31ba1` when you want the action to keep owning tool setup such as Bazel rc files, Maven or Gradle cache config, buildx setup, or container networking.
