...
| [0007](0007-docker-immutable-run-refs-and-alias-promotion.md) | accepted; CI derivation, alias-root binding, and required dual-writer same-alias E2E implemented; default rollout pending CI/action/benchmark proof | Immutable Docker run refs and atomic alias promotion | Provider-neutral same-alias writer E2E is green locally against Rails with two live writer proxies; next gates are CI evidence, signed CLI release, action default path proof, full metadata transport, and benchmark artifacts |
