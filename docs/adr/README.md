...
| [0005](0005-borrowed-upload-sessions-and-blob-cache-policy.md) | accepted for hidden implementation; cache-policy rollout pending benchmark proof | Borrowed upload-session bodies and later blob-cache policy | Large-layer disk-copy/cache-policy evidence; required registry E2E is green at `83e547e` without post-publish blob URL readiness polling |
| [0006](0006-cache-session-trace-and-oci-negative-cache.md) | accepted as first-party insight baseline; backend/action enrichment pending | Session trace, negative cache, and platform insight spine | Backend/action enrichment and artifact validation after the `83e547e` receipt-strict E2E proof |
| [0007](0007-docker-immutable-run-refs-and-alias-promotion.md) | accepted; CI derivation, alias-root binding, and required same-alias E2E harness implemented; default rollout pending CI/backend and benchmark proof | Immutable Docker run refs and atomic alias promotion | Local provider-neutral same-alias writer E2E is green; next proof gate is the same leg in CI with no `blob unknown` publish failure and no verifier-side publish-readiness polling |
