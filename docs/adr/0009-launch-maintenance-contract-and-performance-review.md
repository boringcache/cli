...
Cache scope follows the same ownership boundary. The CLI owns generated
branch/default/PR/platform candidates for archive, proxy adapters, and
Docker/BuildKit refs. The action may pass GitHub metadata and expose
compatibility outputs such as `cache-hit`, `restore-keys`, and readable Docker
import refs, but it must not become a second cache-scope planner. Rails resolves
the ordered tags/aliases clients request and enforces token/write-scope policy.
...
| done | Cache scope model | Make archive, proxy adapters, Docker/BuildKit, action compatibility, and Rails ownership tell one branch/default/PR story | `TagResolver` now owns ordered restore candidates and PR save scope for archive and proxy cache entries. Docker/BuildKit planning follows the same current/base/default model, and action compatibility reads structured CLI check JSON for cache-hit/restore-keys instead of restore exit code. The durable model now lives in the maintained CLI feature inventory under `/Users/gaurav/boringcache/web/.planning/features/cli-*.md`. Local Rails-backed E2E on 2026-05-01 passed archive/core (`scope-model-20260501-archive2`) plus adapter/proxy coverage for config-hints, Gradle, Maven, Docker/BuildKit, OCI same-alias, human-tag restore isolation, Nx, Go, Bazel, and sccache. Turbo was the only selected adapter skip because it is not installed locally. | Ship one product story: CLI owns scope, action orchestrates, Rails resolves requested tags. Do not add PR head-branch reads or global fallback PR writes without an explicit knob and ADR. |
