...
| done | Cache scope model | Make archive, proxy adapters, Docker/BuildKit, action compatibility, and Rails ownership tell one branch/default/PR story | `TagResolver` now owns ordered restore candidates and PR save scope for archive and proxy roots. Docker/BuildKit planning follows the same current/base/default model, and action compatibility reads structured CLI check JSON for cache-hit/restore-keys instead of restore exit code. The durable model is recorded in `docs/comprehension/cache-scope-model.md`. Local Rails-backed E2E on 2026-05-01 passed archive/core (`scope-model-20260501-archive2`) plus adapter/proxy coverage for config-hints, Gradle, Maven, Docker/BuildKit, OCI same-alias, rooted restore isolation, Nx, Go, Bazel, and sccache. Turbo was the only selected adapter skip because it is not installed locally. | Ship one product story: CLI owns scope, action orchestrates, Rails resolves requested tags. Do not add PR head-branch reads or global fallback PR writes without an explicit knob and ADR. |
