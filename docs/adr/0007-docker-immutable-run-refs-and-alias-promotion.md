...
Keep the legacy `buildcache` ref out of the CI-derived alias family. It remains
the local/no-CI compatibility ref, while CI runs use default, branch, and PR
aliases that the corresponding save path can publish.
...
- keep current same-ref behavior for local/no-CI compatibility.
...
Update, 2026-05-05: CI-derived plans no longer append `buildcache` as a stable
read fallback. The GitHub Action readiness gate consumes only the CLI-planned
CI imports and may proceed as soon as the first usable planned ref is readable;
an earlier PR-ref miss is expected on restore-only PRs and should not spend the
whole setup budget before default fallback can be used.
...
- CI-derived import refs no longer include the legacy `buildcache` ref. Derived PR-context promotion refs promote only the PR alias by default, trusted branch/default paths promote only their matching alias, and explicit hidden import/promotion flags can still override the derived plan.
...
- `cache-registry` stores a detected provider-neutral `CiRunContext` on `AppState` once at proxy start. OCI manifest publish and KV flush save requests populate dedicated CI fields only from that context; capped metadata hints remain replay/debug labels and are not enough to create Rails ordering fields.
- CI-backed proxy sessions now auto-infer a low-cardinality `project` metadata hint from repository context when the user did not already set `project` or `benchmark`. Benchmark-mode repositories strip a leading `benchmark-` prefix so shared benchmark workspaces group by workload name instead of repository plumbing.
- Replayable proxy metadata-hint budgeting now favors user-facing grouping labels over per-run noise. Dedicated provider-neutral CI fields remain the ordering contract; replay/debug hints stay intentionally low-cardinality so status, sessions, and misses surfaces can group first-use traffic without extra wiring.
...
- `command_support::save_support::tests::*` covers the first-class save request metadata boundary: local saves stay out of run ordering, generic CI without a run id does not invent ordering fields, and provider-neutral `BORINGCACHE_CI_*` values populate the dedicated save request fields.
...
- PR-context saves promote the PR alias by default.
