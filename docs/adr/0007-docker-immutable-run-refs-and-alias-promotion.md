...
- CLI save/publish request models now carry provider-neutral run metadata (`ci_run_uid`, attempt, ref type/name, default branch, PR number, commit SHA, and run start time) from detected run context, not from capped hints, so Rails can order alias promotions without knowing which CI produced the build.
