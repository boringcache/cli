...
Review outcome as of 2026-04-22: the CLI ADRs are aligned with current CLI `main`, but not all claims are released/product-default evidence yet. The remaining work is now mostly control-plane and proof work: Rails still permits legacy CAS publish without manifest receipts or blob linkage, action/proxy metadata transport must preserve the ordering fields Rails uses for stale-promotion decisions, stream-through and cache admission remain benchmark-gated, and backend-visible session summaries remain shallow compared with the `cache_session_summary` emitted by the proxy.
