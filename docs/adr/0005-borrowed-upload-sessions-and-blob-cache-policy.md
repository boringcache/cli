...
That means the borrowed-session fix has required registry E2E evidence for receipt-strict proxy behavior and same/fresh-reader handoff. It is now released through CLI `v1.12.42` and the public action default, but it is still not cache-policy proof: large-layer admission, eviction, and benchmark evidence remain pending before changing cache admission defaults. Rails now enforces the receipt-strict pending CAS invariant for normal and zero-descriptor manifests; the release gate still needs a fresh CLI/action run after the zero-blob compatibility fix is deployed.
