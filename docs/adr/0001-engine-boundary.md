...
Recent Docker work showed that protocol-native behavior matters. A BuildKit registry cache hit is not just a manifest hit: BuildKit also needs blob bodies. The product default resolves selected OCI refs and lets BuildKit fetch blob bodies on demand through the proxy; `bodies-background` and `bodies-before-ready` remain OCI-specific diagnostic modes, not generic filesystem snapshot behavior.
