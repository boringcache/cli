...
- cache-ops flush now adds a stable `proxy-summary-*` session with `summary_schema` and `summary_json` to the existing Rails `cache-rollups` batch, uses the canonical Rails rollup tool for that proxy (`oci`, `gocache`, `turborepo`, the other KV adapter names, or `runtime`), and shutdown forces one final summary upsert even when there are no fresh flat rollups;
