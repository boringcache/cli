...
| Dashboard | `dashboard [workspace] [--period ...] [--limit ...] [--tag-limit ...] [--interval ...]` | `public-primary`, `lightly-used` | `src/commands/workspace/dashboard.rs` | `ApiClient::workspace_status`, `workspace_tags`, `ratatui`, `crossterm`, `command_support::workspace` | internal layout tests in `src/commands/workspace/dashboard.rs` | Interactive operator surface with lighter coverage than the core CLI workflows; tag panels show named tags only, while raw/internal cache-entry review belongs to the Rails admin audit |
