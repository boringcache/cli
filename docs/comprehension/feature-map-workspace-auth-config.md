...
- ADR 0008 makes the setup lifecycle explicit: `onboard` discovers, `run`/adapters/action execute, and `doctor` plus `audit` maintain `.boringcache.toml` as the repo cache plan.
...
| Doctor | `doctor [workspace] [--json]` | `public-primary`, `lightly-used` | `src/commands/workspace/doctor.rs` | `Config`, `config/source.rs`, `ApiClient::new_for_purpose/get_session_info`, `command_support::workspace` | help coverage in `tests/integration_tests.rs`, unit tests in `src/commands/workspace/doctor.rs` | Main diagnostic consumer of token-source and api-url-source helpers; launch maintenance check for workspace/token/config health; API diagnostics report the normalized v2 URL only |
| Dashboard | `dashboard [workspace] [--period ...] [--limit ...] [--tag-limit ...] [--interval ...]` | `public-primary`, `lightly-used` | `src/commands/workspace/dashboard.rs` | `ApiClient::workspace_status`, `workspace_tags`, `ratatui`, `crossterm`, `command_support::workspace` | internal layout tests in `src/commands/workspace/dashboard.rs` | Interactive operator surface with lighter coverage than the core CLI workflows; tag panels collapse the replica-safe tag feed to primary human tags, sort by last access/upload time, and paginate locally; the Sessions side panel reuses status helpers for run labels, native tool stats, and customer-safe review lines; raw/internal cache-entry review belongs to the Rails admin audit |
...
| Onboard project and account | `onboard [path] [--workspace ...] [--create-workspace] [--create-ci-tokens] [--github-secrets] [--github-repo ...] [--rotate-ci-tokens] [--workspace-name ...] [--email ...] [--name ...] [--username ...] [--apply] [--dry-run] [--manual] [--json]` | `public-primary` | `src/commands/workspace/onboard.rs` | `ApiClient` CLI-connect, workspace provision, CI token, and optimize calls; `Config`, `audit`, `project_config`, `optimize`, `ui`, `gh` for optional GitHub secret writes | `tests/onboard_command_tests.rs`, `tests/cli_workflow_tests.rs`, `README.md`, `docs/onboard.md`, `docs/github-actions.md` | This is the setup funnel and one of the biggest orchestration files; writes the first repo cache plan when possible. Launch automation can now noninteractively verify/create a workspace, create split CI tokens, set GitHub repository secrets without printing token values, write/verify repo workspace config even when CI files need no optimization, and emit a JSON onboarding report with bounded `next_steps` for automation handoff. Interactive auth prints both the manual code-entry URL and direct approval URL, and automatic browser launch uses the direct approval URL. Unlike `login`, onboarding requests a workspace-scoped CLI-connect token because the flow is a project/workspace connector. |
| Audit repo cache config | `audit [root] [--path PATH]... [--write] [--json]` | `public-primary` | `src/commands/workspace/audit.rs` | `project_config::{discover,normalize_profile_name,canonical_entry_id}`, `parse_save_format`, `jwalk`, `ui` | `tests/audit_command_tests.rs`, `docs/onboard.md` | Migration/discovery helper and long-term rescan path for `.boringcache.toml`; default scanning includes `.github/workflows`, and `boringcache/one@v1` workflow `entries` are imported as stable repo entries while `cache-tag` stays runtime scoping; future `lint` or `rescan` UX should wrap this planner/audit path instead of creating a second config truth |
...
| `src/api/client/workspace.rs` and related models | workspace provision/list/status/session/token/optimize server calls |
...
- `ApiClient` normalizes configured API URLs to v2 and does not keep a legacy API fallback base.
- Launch maintenance should keep `doctor --json` and `audit --json` cheap enough for CI drift checks. If public copy says "self-healing" or "rescan", tie it back to these commands and avoid implying background mutation without user review.
