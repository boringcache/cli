...
| Doctor | `doctor [workspace] [--json]` | `public-primary`, `lightly-used` | `src/commands/workspace/doctor.rs` | `Config`, `config/source.rs`, `ApiClient::new_for_purpose/get_session_info`, `command_support::workspace` | help coverage in `tests/integration_tests.rs`, unit tests in `src/commands/workspace/doctor.rs` | Main diagnostic consumer of token-source and api-url-source helpers; launch maintenance check for workspace/token/config health |
