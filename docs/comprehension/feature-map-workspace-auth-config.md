...
| Audit repo cache config | `audit [root] [--path PATH]... [--write] [--json]` | `public-primary` | `src/commands/workspace/audit.rs` | `project_config::{discover,normalize_profile_name,canonical_entry_id}`, `parse_save_format`, `jwalk`, `ui` | `tests/audit_command_tests.rs`, `docs/onboard.md` | Migration/discovery helper and long-term rescan path for `.boringcache.toml`; future `lint` or `rescan` UX should wrap this planner/audit path instead of creating a second config truth |
