...
- CI-derived Docker imports keep `buildcache` as a stable read fallback during migration, but PR-context saves should promote only the PR alias by default. Promoting the global fallback from PR context needs an explicit override and a matching Rails token/write-scope decision.
