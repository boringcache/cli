...
- `ci_detection` is active through save/mount/CAS publish request metadata, telemetry tagging, and Docker human-tag planning; it owns provider-neutral `BORINGCACHE_CI_*` run metadata plus GitHub Actions mapping
...
- `src/serve/cache_registry/tool_routes/mod.rs`
- `src/serve/cache_registry/tool_routes/bazel.rs`
- `src/serve/cache_registry/tool_routes/go_cache.rs`
- `src/serve/cache_registry/tool_routes/gradle.rs`
- `src/serve/cache_registry/tool_routes/maven.rs`
- `src/serve/cache_registry/tool_routes/nx.rs`
- `src/serve/cache_registry/tool_routes/sccache.rs`
...
- `ci/e2e/required/e2e-oci-human-tag-restore-isolation-test.sh`
...
- `e2e-local-adapter-commands.sh` can select `LOCAL_ADAPTER_TOOLS=dual-proxy` to run the extended dual-proxy contention script against the local Rails-provisioned workspace and tokens. The local Rails-backed dual-proxy path passed on 2026-05-04 with status-backed merged-index verification.
- `e2e-dual-proxy-contention-test.sh` treats `/_boringcache/status` as the durable merged-index verification surface. Its final proxy writes `verify/proxy-status.json` and derives the loaded-index assertion from startup prefetch metrics, with debug log scraping only as compatibility fallback.
- `e2e-oci-human-tag-restore-isolation-test.sh` covers human-tag scoped OCI alias isolation; new CLI Docker/BuildKit paths use resolved human tags directly for normal registry-cache refs.
