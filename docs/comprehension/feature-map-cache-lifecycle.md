...
| Check cache presence | `check WORKSPACE TAGS` with `--no-platform`, `--no-git`, `--fail-on-miss`, `--exact`, `--json` | `public-primary` | `src/commands/cache/check.rs` | `command_support`, `tag_utils`, `platform`, `git`, `ApiClient`, `signing::policy` | `tests/integration_tests.rs`, `tests/api_contract_tests.rs` | Read-side verification surface. JSON hit rows include `cache_entry_id` and `manifest_root_digest` so E2E handoffs can prove aliases resolve to the confirmed root through the normal restore path rather than a separate pointer-only API probe. |
