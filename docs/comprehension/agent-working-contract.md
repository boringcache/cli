...
- Proxy KV and Docker/BuildKit publish paths use the resolved human tag as the cache head. New CLI code must not derive generated protocol aliases as first-class cache tags, and the backend should treat syntactically valid tag names literally rather than assigning prefix semantics.
