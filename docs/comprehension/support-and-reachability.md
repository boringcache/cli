...
| Signing policy | restore/check/mount verification | `support-primary` | `src/signing/mod.rs`, `src/signing/policy.rs` | Warn-only by default; strict mode is opt-in. Canonical server signature payloads are verified when present, and `BORINGCACHE_TRUSTED_WORKSPACE_KEY_FINGERPRINT` adds an optional pinned workspace-key trust boundary. |
