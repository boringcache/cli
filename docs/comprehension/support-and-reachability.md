...
| API client and models | nearly every command and serve path | `support-primary` | `src/api/client/mod.rs`, `http.rs`, `cache.rs`, `auth.rs`, `workspace.rs`, `metrics.rs`, `src/api/models/**` | The main transport and DTO layer; large hotspot with mixed command concerns. Rails API calls normalize to v2 and capability negotiation no longer carries a legacy fallback chain. |
