...
- Local main also carries focused proxy coverage for the historical ADR 0007 alias shape. As of the 2026-05-13 revision, that evidence is compatibility/race context; new Docker/BuildKit planning uses resolved human tags directly.
...
- The required E2E matrix now includes a provider-neutral OCI same-alias writer leg. It uses the standalone proxy's hidden alias-promotion hook with two live writer proxies, publishes a newer human ref before an older ref, requires promoted and `ignored_stale` alias results with zero promotion failures, and verifies both human refs plus the winning alias through a fresh proxy.
...
- Concurrency follow-up:
  ...
  - Local Rails/Tigris-backed E2E with 5,000 x 4 KiB blobs measured startup-prefetch readiness at 36s with concurrency 3, 13s with concurrency 10, and 14s with concurrency 20. That earlier small local sweep was not representative of high-latency many-small-object benchmark tags.
  - The 2026-05-01 gRPC Bazel rolling tag had 16,536 blobs / 745 MiB, about 47 KiB per blob. GitHub Actions at concurrency 10 spent 108.2s in startup hydration. A wider sweep over the same tag found 50 -> 39.5s, 100 -> 27.3s, 200 -> 23.2s, and 400 -> 20.3s with zero errors, putting the useful knee around 100. For this shape, small blobs are RTT-bound and need more startup-prefetch concurrency, not less.
  - Startup prefetch now has a separate resource-scaled CI budget from steady-state blob downloads. Capable CI runners can prefetch many-small-object tags with an adaptive controller that starts at 20 and probes up to concurrency 100 when goodput improves without failures, while normal proxy read-through/download concurrency remains lower and large/medium blob classes still keep conservative caps.
