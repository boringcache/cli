...
- The required E2E matrix now includes a provider-neutral OCI same-alias writer leg. It uses the standalone proxy's hidden alias-promotion hook to publish a newer immutable ref, publish an older ref second, require an `ignored_stale` alias result, and verify both immutable refs plus the winning alias through a fresh proxy.
- Local Rails-backed direct OCI same-alias writer E2E passed on 2026-04-22 after keeping proxy metadata hints within the replayable 8-key cap. Docker/OCI full-build local E2E was not rerun because the local Docker daemon was unavailable in that environment.
