...
- The request metrics summarizer now promotes `oci_engine` status keys from both `proxy-status-*.json` and real-run `status-*.json` snapshots into `request-metrics-summary.env`, so BuildKit E2E and ad hoc project artifacts can be studied after release without hand-parsing proxy status JSON.
