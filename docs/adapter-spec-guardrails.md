...
| Nx custom remote cache | Nx self-hosted custom remote cache OpenAPI | `/v1/cache/{hash}`, `/v1/terminalOutputs/{hash}`, and `POST /v1/cache` query | bearer auth test, artifact round trip, `test_nx_artifact_get_miss_returns_not_found`, query miss test, duplicate artifact upload conflict test, terminal output miss test | Current Nx OpenAPI documents the artifact route; terminal output and query support are retained for observed tool traffic and should be rechecked before a broader Nx engine boundary |
