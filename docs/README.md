# CLI docs

The CLI README is the entrypoint.

If you are new to BoringCache, start here:

- [Quick start](quick-start.md)
- [Onboard](onboard.md)
- [Archive mode](archive-mode.md)
- [Adapter commands](adapter-commands.md)
- [Tool guides](tool-guides.md)
- [GitHub Actions](github-actions.md)

If you are working on the CLI repo itself:

- [Development](development.md)
- [Adapter cache profiles](adapter-cache-profiles.md)
- [Adapter contract matrix](adapter-contract-matrix.md)
- [Adapter spec guardrails](adapter-spec-guardrails.md)
- [Historical performance learning log](performance-learning-log.md)
- [Engine boundary ADR](adr/0001-engine-boundary.md)
- [Proxy engine Plan B ADR](adr/0002-proxy-engine-plan-b.md)
- [Runner proxy optimization roadmap ADR](adr/0003-runner-proxy-optimization-roadmap.md)
- [OCI large blob stream-through ADR](adr/0004-oci-large-blob-stream-through.md)
- [Borrowed upload sessions and blob cache policy ADR](adr/0005-borrowed-upload-sessions-and-blob-cache-policy.md)
- [Cache session trace and OCI negative cache ADR](adr/0006-cache-session-trace-and-oci-negative-cache.md)
- [Docker registry-cache identity ADR](adr/0007-docker-human-tag-registry-identity.md)
- [OCI mistake ledger](oci-mistake-ledger.md)
- [sccache mistake ledger](sccache-mistake-ledger.md)
- [Bazel mistake ledger](bazel-mistake-ledger.md)
- [Gradle/Maven mistake ledger](gradle-maven-mistake-ledger.md)
- [Installation setup](../INSTALLATION.md)

The website docs live at [boringcache.com/docs](https://boringcache.com/docs).
