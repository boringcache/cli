...
| Maven / HTTP method set | Maven HEAD support must not be treated as optional while sharing Gradle-like KV substrate. | `serve::engines::maven` | Closed for the current Maven extension behavior: Maven owns `GET`/`HEAD`/`PUT` dispatch. A 2026-04-22 source audit found Resolver `GetTask`/`PutTask` behavior with not-found reads as misses and logged remote-save failures, but no Maven-specific upload rejection status, so generic KV backlog rejection remains intentional. |
...
The first Gradle/Maven boundary keeps route handling in `src/serve/cache_registry/tool_routes/{gradle,maven}.rs` as glue while moving method/status ownership into `src/serve/engines/{gradle,maven}.rs`. The 2026-04-22 Maven source audit found no Maven-specific upload-rejection status beyond Resolver storage behavior, so Maven intentionally keeps generic KV write rejection while Gradle maps oversized writes to official `413`. The remaining acceptance list should keep covering Gradle hit/miss/write statuses, Maven `HEAD`/`GET`/`PUT`, exact key paths, opaque payload bytes, and local Gradle/Maven E2E runs against the Rails-backed proxy.
