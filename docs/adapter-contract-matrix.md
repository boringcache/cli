...
| --- | --- | --- | --- |
| OCI / blob reads | Blob pulls used to ignore `Range` and return full `200` bodies. | `serve::engines::oci::blobs` extraction | Blob engine now owns `Accept-Ranges`, partial `206`, invalid `416`, `If-Range`, remote read-through, and digest/size verification. Full BuildKit parity still requires the range/status acceptance tests and matrix E2E before closing this row. |
...
| OCI / engine boundary | OCI must not be treated as generic KV. Registry truth is manifest/blob/session/referrer shaped. | OCI/KV path audit before manifest and blob engine moves | Classify every touched `cache_registry/kv` path as protocol-neutral substrate or OCI behavior that belongs in the engine; keep `docs/oci-kv-path-audit.md` current as that boundary moves. |
