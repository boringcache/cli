...
| Nx | Nx custom remote cache OpenAPI | `/v1/cache/{hash}` | Task output tar archives by hash | OpenAPI-defined download | OpenAPI-defined upload | Auth errors are explicit 401/403 text responses; artifact misses are `404`; existing artifact uploads are `409` | Treat payload as Nx task-output archive bytes; do not inspect or mutate archive format, and do not overwrite an existing artifact hash |
