...
| Bazel / AC-CAS boundary | Bazel AC/CAS truth should not live as Bazel-specific branches in generic KV. | `serve::engines::bazel` | Started: Bazel store identity maps `/ac/` and `/cas/` onto distinct KV namespaces, and the CAS integrity policy verifies SHA-256 body digests while action-cache payloads stay opaque. KV accepts a protocol-neutral integrity hook and remains the storage substrate. |
