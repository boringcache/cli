...
// Global mutex to ensure CLI tests run sequentially to avoid environment variable interference
static CLI_TEST_MUTEX: Mutex<()> = Mutex::new(());

// Helper function to acquire lock with poison recovery
fn acquire_test_lock() -> MutexGuard<'static, ()> {
    match CLI_TEST_MUTEX.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
            // Recover from poisoned mutex by clearing the poison and continuing
            poisoned.into_inner()
        }
    }
}
...
async fn test_auth_workflow_success() {
    let _lock = acquire_test_lock(); // Ensure sequential execution
