...
# Usage: curl -sSL -H "Cache-Control: no-cache" -H "Pragma: no-cache" https://install.boringcache.com | sh
...
install_binary() {
    ...
    case "${os}-${arch}" in
        "darwin-amd64")
            # Use macOS 13 for Intel Macs (best compatibility)
            binary_name="boringcache-macos-13-amd64"
            ;;
        "darwin-arm64")
            # Use macOS 15 for Apple Silicon (latest)
...
main() {
    ...
    echo
    print_status "2. Save cache (path:tag format):"
    print_status "   ${BINARY_NAME} save my-workspace \"node_modules:node-deps,target:build-cache\""
    echo
